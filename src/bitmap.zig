            ...
            switch (container) {
                .array => |ac| {
                    ...
                    if (n_runs * 4 < @as(u32, ac.cardinality) * 2) {
                        const rc = try arrayToRunContainer(self.allocator, ac);
                        ac.deinit(self.allocator);
                        self.containers[i] = TaggedPtr.initRun(rc);
                        converted += 1;
                    }
                },
                .bitset => |bc| {
                    ...
                    if (n_runs * 4 < 8192) {
                        const rc = try bitsetToRunContainer(self.allocator, bc);
...
pub const FrozenBitmap = struct {
    ...
    pub fn init(data: []const u8) !Self {
        ...
        if ((cookie & 0xFFFF) == RoaringBitmap.SERIAL_COOKIE) {
            has_runs = true;
            size = ((cookie >> 16) & 0xFFFF) + 1;
            const bitset_bytes = (size + 7) / 8;
            ...
            pos += bitset_bytes;
        } else if (cookie == RoaringBitmap.SERIAL_COOKIE_NO_RUNCONTAINER) {
